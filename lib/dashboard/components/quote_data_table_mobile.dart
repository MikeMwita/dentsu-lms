...
class QuoteDataTable extends StatefulWidget {
  /// Creates the home page.
  const QuoteDataTable({super.key});

  @override
  State<QuoteDataTable> createState() => _QuoteDataTable();
}
...
class _QuoteDataTable extends State<QuoteDataTable> {
          ...
          child: SfDataGrid(
            ...
            onCellTap: ((details) {
              if (details.rowColumnIndex.rowIndex != 0) {
                ...
                context.go('/quotes_mobile/quote_details', extra: data);
              }
            }),
            columns: <GridColumn>[
              GridColumn(
  ...
  List<Lead> getEmployeeData() {
    return [
      ...
      Lead('20', 'Grimes Developer'),
      Lead('21', 'Grimes Developer'),
      Lead('22', 'Grimes Developer'),
      Lead('23', 'Grimes Developer'),
      Lead('24', 'Grimes Developer'),
      Lead('25', 'Grimes Developer'),
      Lead('26', 'Grimes Developer'),
      Lead('27', 'James Developer')
    ];
  }
...
class EmployeeInfoDataSource extends DataGridSource {
  ...
  Future<bool> handlePageChange(int oldPageIndex, int newPageIndex) async {
    ...
    if (startIndex < _employees.length) {
      // Ensure endIndex does not exceed the total number of employees
      endIndex = endIndex <= _employees.length ? endIndex : _employees.length;
